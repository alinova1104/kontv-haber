...
import { debounce } from 'lodash'; // lodash'dan debounce'u içe aktarıyoruz
...
export default function EditPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log(datas);
        setInitialData(datas); // Başlangıç verilerini sakla
      } catch (error) {
  ...
  const debouncedCheckIfFormChanged = useCallback(
    ...
    [faxs, phones, emails, address, googleMapsLink, initialData]
  );


  useEffect(() => {
    debouncedCheckIfFormChanged();
    return () => debouncedCheckIfFormChanged.cancel();
  }, [faxs, phones, emails, address, googleMapsLink, debouncedCheckIfFormChanged]);
